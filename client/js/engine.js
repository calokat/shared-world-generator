...
function moveCamera(e) {
  ...
  if (e.key == "w") {
    camera.position.add(cameraFwd.multiplyScalar(0.1));
  }
  if (e.key == "s") {
    camera.position.add(cameraFwd.multiplyScalar(-0.1));
  }
  if (e.key == "a") {
    camera.position.add(cameraRight.multiplyScalar(-0.1));
  }
  if (e.key == "d") {
    camera.position.add(cameraRight.multiplyScalar(0.1));
  }
...
function addEntity(geometry, newName) {
  ...
  mesh.position
    .add(camera.position)
    .add(camera.getWorldDirection(cameraFwd).multiplyScalar(5));
  mesh.name = newName;
  entities.push(mesh);
  scene.add(mesh);
}
...
renderer.domElement.addEventListener("mousedown", (e) => {
  ...
  for (; firstMeshIndex < intersects.length; ++firstMeshIndex) {
    ...
    if (intersects[firstMeshIndex].object.type == "Mesh") {
      ...
      selectedOutline = new THREE.LineSegments(
        selected.geometry,
